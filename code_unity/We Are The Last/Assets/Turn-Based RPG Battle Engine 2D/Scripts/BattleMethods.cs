    ...
    foreach ( InstanceID target in context.actionTargets )
    {
      ...
      if ( attribute.curValue > 0 )
      {
        ...
        if ( counterSourceIndex >= 0 )
        {
          ...
          characterInfo counterActor = BattleManager.core.characterInstances.FirstOrDefault( x => x.characterInstanceId.CharacterID == Mathf.FloorToInt(instanceIdValue) );
 ...
 */
    ...
    bool friendlyEffects, int superPerEffect )
  {
    ...
    List<string> hostile = new List<string> (){ "POISON", "STUN", "PARALYZE", "DOOM", "DOOMSOURCE", "STRESS", "STRESSPOWER" };
