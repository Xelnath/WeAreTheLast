...
namespace GoogleSheetsForUnity
{
    public static class Drive
    {
        ...
        public static ConnectionData ConnectionData { get { return _connectionData; } set { _connectionData = value; } }
        ...
        /// </summary>
        /// <param name="runtime">Bool value indicating if the request was sent from Unity Editor or running game.</param>
        public static void GetTableNames(bool runtime = true)
        {
            Dictionary<string, string> form = new Dictionary<string, string>();
            form.Add("action", QueryType.getTableNames.ToString());
        ...
        private static void SetConnectionData(bool runtime)
        {
            ...
            if (_connectionData == null)
            {
                var assets = AssetDatabaseHelper.FindAssetsByType<ConnectionData>();
                if (assets.Count > 0)
                    _connectionData = assets[0];
            }
        ...
        private static void CreateRequest(Dictionary<string, string> dataForm, bool runtime)
        {
            ...
            // Does this work in both editor and not?
            DriveConnection.ExecuteRequest(www, form);
            /*
                        if (runtime)
                        {
                            DriveConnection.ExecuteRequest(www, form);
